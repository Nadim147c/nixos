    ...
    }:
    {
      vim = {
        ...
        lsp = {
          ...
          servers.nimlangserver = {
            inherit (config.vim.lsp.servers.nimlsp) root_dir;
            filetypes = singleton "nim";
            cmd = singleton <| getExe pkgs.nimlangserver;
          };
          servers.nimlsp.enable = mkForce false;
          servers.nixd = {
            settings.options = {
              nixos.expr = /* nix */ ''
                  let
