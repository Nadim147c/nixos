...
void main() {
    ...
    vec2 pixelizedUV = floor(qt_TexCoord0 * (size / pixelSize)) * (pixelSize / size);
    ...
    if (distMain <= -borderWidth * pixelSize) {
        vec2 imageUV = pixelizedUV * (size / boxSize);
        color = texture(imageTexture, imageUV);
    }
    else if (distMain <= 0.0) {
        color = borderCol;
    }
